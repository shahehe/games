...
- (void) runTo:(CGPoint)position withDuration:(ccTime)_duration
{
    BOOL isForward = YES;
    ...
    self.position = ccp(self.position.x, position.y);
    CCMoveTo *move = [CCMoveTo actionWithDuration:_duration position:position];
    [self runAction:move];
